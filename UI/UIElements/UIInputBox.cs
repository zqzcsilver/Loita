...
{
    internal delegate void TextChangeEvent(UIInputBox inputBox, string text);
    ...
    internal class UIInputBox : UIPanel
    {
        ...
        private float symOffsetX;
        private Vector2 _symSizeOffset;
        ...
        public float FontSize
        {
            ...
            set
            {
                ...
                var c = Font.MeasureString(cursorSym[0].ToString()) + _symSizeOffset;
        ...
        private SpriteFontBase Font => Loita.DefaultFontSystem.GetFont(_fontSize);
        public bool IsEnableIME => isEnableIME;

        public UIInputBox(string text = "", float fontSize = 40f, Point cursorPosition = default(Point), Color color = default(Color), Vector2 symSizeOffset = default)
        {
            _text = text;
            _fontSize = fontSize;
            ...
            offset = Vector2.Zero;
            symHitBox = Rectangle.Empty;
            _symSizeOffset = symSizeOffset;
            var c = Font.MeasureString(cursorSym[0].ToString()) + symSizeOffset;
            symHitBox.Width = (int)c.X;
            symHitBox.Height = (int)c.Y;
            symOffsetX = c.X / 2f;
        }
        ...
        public override void LoadEvents()
        {
            ...
            Events.OnLeftClick += (element) =>
            {
                ...
                {
                    if (mousePos.Y > symHitBox.Height)
                        mousePos.Y -= symHitBox.Height;
                    else
                        break;
        ...
        {
            var texts = Text.Split('\n');
            float lineHeight = Font.MeasureString("啊").Y;
            float offsetY = 0f;
            string text;
            int j = timer % 14;
            float x;
            var us = ModContent.GetInstance<UISystem>();
            for (int i = 0; i < texts.Length; i++)
            {
                text = texts[i];
                if (isEnableIME && i == CursorPosition.Y && (j <= 8 && j >= 0))
                {
                    x = Font.MeasureString(text.Substring(0, CursorPosition.X)).X;
                    ...
                    {
                        us.IMEPosition = Info.Location + new Vector2(x - symOffsetX, offsetY + symHitBox.Height * 2f) + offset;
                    }
                }
